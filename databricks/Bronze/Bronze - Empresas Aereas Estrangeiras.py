...
# MAGIC %md
# MAGIC # Camada Bronze - Empresas Aéreas Estrangeiras
# MAGIC
# MAGIC Este pipeline ingere dados de referência de empresas aéreas estrangeiras na camada bronze.
# MAGIC
# MAGIC **Origem**: `/Volumes/mizukiairflows/bronze/arquivos/Referencias/pda_empresas_aereas_estrangeiros.csv`
# MAGIC
# MAGIC **Destino**: `mizukiairflows.bronze.empresas_aereas_estrangeiras`
# MAGIC
...
# Criar tabela bronze para empresas aéreas estrangeiras

print("Criando tabela bronze para empresas aéreas estrangeiras...\n")

spark.sql("DROP TABLE IF EXISTS mizukiairflows.bronze.empresas_aereas_estrangeiras")

create_table_sql = """
CREATE TABLE mizukiairflows.bronze.empresas_aereas_estrangeiras (
  _source_file STRING COMMENT 'Caminho do arquivo de origem',
  _ingestion_timestamp TIMESTAMP COMMENT 'Timestamp de ingestão'
)
USING DELTA
COMMENT 'Camada Bronze: Empresas aéreas estrangeiras - todas colunas como STRING'
TBLPROPERTIES (
...
volume_path = "/Volumes/mizukiairflows/bronze/arquivos/Referencias/pda_empresas_aereas_estrangeiros.csv"

print(f"Iniciando ingestão...")
print(f"Origem: {volume_path}")
print(f"Destino: mizukiairflows.bronze.empresas_aereas_estrangeiras")
...
try:
    ...
    temp_path = "/Volumes/mizukiairflows/bronze/arquivos/_temp/empresas_estrangeiras_clean.csv"
    df_without_metadata.coalesce(1).write.mode("overwrite").text(temp_path)
    
    # Agora ler o arquivo limpo com COPY INTO
    result = spark.sql(f"""
    COPY INTO mizukiairflows.bronze.empresas_aereas_estrangeiras
    FROM (
      SELECT 
        *,
...
# MAGIC FROM mizukiairflows.bronze.empresas_aereas_estrangeiras;
# MAGIC
# MAGIC -- Amostra
# MAGIC SELECT * FROM mizukiairflows.bronze.empresas_aereas_estrangeiras LIMIT 10;
